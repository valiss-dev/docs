...
requests. A **credentials file** is those two things packaged together in one
...
operator produces credentials files with the `creds` package or `examples/minter`,
which writes a credentials file straight from a manifest.

## The marker-delimited layout

A credentials file is line-oriented and delimited by begin and end markers, the same
...
their own wire version. Parsing is strict. Each present section
...
A user-level credentials file has a choice to make about the account token. The chain
...
shape the message-token transports require, since an emitter issues
...
in the credentials file is the thing to guard.
